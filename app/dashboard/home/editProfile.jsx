...
const EditProfilePage = () => {
  ...
  const saveNewPassword = async () => {
    if (newPassword === oldPassword) return showAlert("Can't be the same password", false);
    ...
    const success = await updateServerData(data);
    if (success) {
      showAlert("Successfully updated password.", true);
      setOldPassword("");
      setNewPassword("");
    }
            ...
            <ScrollView showsVerticalScrollIndicator={false} style={{flex: 1,}} contentContainerStyle={{paddingBottom: 120}}>
              ...
              <View style={{width: screenWidth,  justifyContent: "center", alignItems: "center"}}>
                ...
                <View style={{width: "100%"}}>
                  <ThemedTextInput value={username} placeholder='Change Username' onChange={(v) => {setUsername(v.split('').splice(0, 15).join(''))}} />
                    {/* Svae button */}
                ...
                <ThemedText style={{fontSize: 15, fontWeight: 700, marginBottom: 25}}>Edit Personal Information</ThemedText>


                <View style={{width: "100%"}}>
                  <ThemedTextInput value={email} placeholder='Change Email' onChange={(v) => {setEmail(v)}} />
                    {/* Svae button */}
                    {email !== user.email && (
                    <Animated.View entering={SlideInRight.springify().damping(90)} exiting={SlideOutRight.springify().damping(90)} style={{position: 'absolute', right: 20, top: 0, bottom: 0, zIndex: 2, justifyContent: "center"}}>
                      <Pressable onPress={saveEmail} style={{padding: 20, backgroundColor: Colors.primaryBlue,borderRadius: 10 }}>
                        {loadingEmail ? (<View>
                          <Loading style={{width: 20, height: 20,}} />
                        </View>) : (<View style={{width: "100%", justifyContent: "center", alignItems: "center"}}>
                          <Text style={{color: "white", textAlign: "center"}}>Save</Text>
                ...
                <Spacer height={20} />
                <ThemedText style={{fontSize: 15, fontWeight: 700, marginBottom: 20, marginRight: "auto", paddingHorizontal: 30}}>Change Password</ThemedText>

                <View style={{width: "100%"}}>
                  <ThemedTextInput value={oldPassword} placeholder='Old Password' onChange={(v) => {setOldPassword(v)}} type='password'  />
                    <Spacer height={20} />
                  <ThemedTextInput value={newPassword} placeholder='New Password' onChange={(v) => {setNewPassword(v)}} type='password' />
                    <Spacer height={10} />
                    {(oldPassword && newPassword.length >= 8) && <Animated.View entering={FadeIn} exiting={FadeOut} style={{width: "100%"}}>
                      <BlueButton onPress={saveNewPassword} style={{marginHorizontal: 30}} title={loadingPassword ? "Loading..." : "Update Password"} />
                    </Animated.View>}
